% =========================================================================
% 脚本名称：零序电流比幅法 — 准确性批量测试
% 文件名：  Amplitude_Compare_Accuracy_Analysis.m
% 适用模型：Distribution_Fault_Model.slx
% -------------------------------------------------------------------------
% 功能说明：
%   自动遍历多种故障电阻、故障初相角、消弧线圈补偿度的组合工况，
%   对每种工况运行仿真并执行比幅法选线判据，统计算法的选线准确率。
%
% 核心原理：
%   故障馈线的零序电流 = 全系统非故障馈线对地电容电流之和（方向相反），
%   其 RMS 幅值远大于任何一条健全馈线。
%   判据：稳态零序电流 RMS 最大的馈线 = 故障馈线。
% =========================================================================
...
%   初相角 α 对应 SwitchTimes = t_base + (α/360) * T
%   t_base = 0.1s (第5个周期结束), T = 0.02s (工频周期)
alpha_list = [0, 30, 45, 60, 90, 150];   % 度
t_base = 0.1;
T_period = 1/50;
switch_times_list = t_base + (alpha_list / 360) * T_period;

% --- 消弧线圈补偿度 ---
%   全补偿电感 L_full = 1/(ω²·C_total) = 1.0262 H
%   脱谐度 v: 欠补偿 +10%, 全补偿 0%, 过补偿 -10%
L_full = 1.0262;
compensation_labels = {'欠补偿(v=+10%)', '全补偿(v=0%)', '过补偿(v=-10%)'};
L_coil_list = [L_full / 0.90, ...   % 欠补偿: L = L_full/(1-v) = L_full/0.9
               L_full, ...           % 全补偿
               L_full / 1.10];       % 过补偿: L = L_full/(1+|v|) = L_full/1.1

% --- 比幅法参数 ---
fault_threshold = 2.0;     % 比率门槛阈值
steady_delay = 0.045;      % 故障后等待时间 (s)，跳过暂态
steady_duration = 0.040;   % 稳态窗口持续时间 (s)，约 2 个工频周期
fs = 20000;                % 采样频率 (Hz)
...
results.rms_values = zeros(total_cases, num_feeders);
results.ratio = zeros(total_cases, 1);

%% ======================== 加载模型 ========================

fprintf('========================================\n');
fprintf('  比幅法准确性批量测试 开始\n');
fprintf('  总工况数: %d\n', total_cases);
fprintf('========================================\n\n');
    ...
    for i_rf = 1:length(Rf_list)
        ...
        for i_alpha = 1:length(alpha_list)
            ...
            try
                ...
                % --- 计算各馈线零序电流 RMS ---
                rms_vals = [rms(i1(idx)), rms(i2(idx)), rms(i3(idx)), rms(i4(idx))];
                
                % --- 选线判据：RMS 最大 + 比率门槛 ---
                [max_rms, detected] = max(rms_vals);
                ratio_val = max_rms / mean(rms_vals);
            ...
            results.rms_values(case_idx, :)  = rms_vals;
...
result_table = table(results.compensation, results.Rf, results.alpha, ...
    ...
    results.rms_values(:,1), results.rms_values(:,2), ...
    results.rms_values(:,3), results.rms_values(:,4), ...
    'VariableNames', {'补偿状态', 'Rf_Ohm', '初相角_deg', ...
    '检测馈线', '是否正确', '比率', ...
    'RMS_馈线1', 'RMS_馈线2', 'RMS_馈线3', 'RMS_馈线4'});

% 保存结果
save_path = fullfile(fileparts(which(mfilename)), 'Amplitude_Compare_Results.mat');
save(save_path, 'result_table', 'results', 'total_accuracy');
fprintf('\n结果已保存至: %s\n', save_path);

%% ======================== 可视化：准确率热力图 ========================

figure('Name', '比幅法 — 准确率热力图', 'NumberTitle', 'off', ...
...
for i_comp = 1:length(compensation_labels)
    ...
    imagesc(acc_matrix);
    colormap(gca, [0.9 0.3 0.3; 0.3 0.8 0.3]);  % 红=错误, 绿=正确
...
sgtitle(sprintf('比幅法选线准确率热力图 (总准确率: %.1f%%)', total_accuracy), ...
